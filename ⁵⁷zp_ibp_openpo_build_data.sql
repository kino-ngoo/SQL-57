...
ALTER PROCEDURE [dbo].[zp_ibp_openpo_build_data]        
...
/*                     
   SELECT @XML_INPUT = '<sb_request_context>                      
                           <inputstring>                      
                             <duns_no>0054</duns_no>              
                             <subject>TEST</subject>               
                             <flow_code>H_INVT</flow_code>              
                             <center_duns_no>0001</center_duns_no>              
                           </inputstring>                      
                        </sb_request_context>'              

   SELECT @XML_INPUT ='<sb_request_context><inputstring><id_admcomp>2</id_admcomp></inputstring></sb_request_context>'                                      
*/   

--[20]--- Extract Data From Request --------------------------------------------------------------------------------------------------------------
   SELECT @xml_request = CAST( dbo.fnReplTag( CAST( @xml_request AS nvarchar(MAX) ) ) AS xml )
--[20]--- Extract Data From Request --------------------------------------------------------------------------------------------------------------
  DECLARE @loop_count int ,@loop_index int ,@stat_b2b_next TINYint ,@wf_msg nvarchar(MAX) ,@actioncode varchar(10) ,@inputstring nvarchar(MAX)                
  DECLARE @table_list nvarchar(MAX) ,@para_list nvarchar(MAX)
  DECLARE @input_xml xml                
   SELECT @wf_msg = CAST(T.c.query('.') AS nvarchar(MAX)) FROM @XML_INPUT.nodes('/sb_request_context/wf') T(c)
   SELECT @wf_msg = isnull(@wf_msg, '<wf></wf>')
   SELECT @inputstring = CAST(T.c.query('.') AS nvarchar(MAX)) FROM @XML_INPUT.nodes('/sb_request_context/inputstring') T(c)
 ...
 --SELECT '@filename',@filename,'@msg_type',@msg_type
   ...
   SELECT id_scvo_item_master,dt_release = MAX(dt_release)  
    ...
    --AND DATEPART(quarter,dt_valid_from)=DATEPART(quarter,GETDATE()) -- Jill mark on 2013-06-28  
      AND CONVERT(varchar(10),GETDATE(),121) Between CONVERT(varchar(10),dt_valid_from,121) and CONVERT(varchar(10),dt_valid_to,121) -- Jill add on 2015-01-07
...
/*   
   IF exists(select 1    
       ...
       WHERE --CONVERT(varchar(10),dt_generate,121) = CONVERT(varchar(10),dateadd(d,0,getdate()),121)  
             CONVERT(varchar(10),dt_generate,121) = CONVERT(varchar(10),getdate(),121)  
         AND stat_void=0)  
       BEGIN  
...
/*  
   SELECT *   
    ...
    WHERE --CONVERT(varchar(10), dt_generate,121)  
         ,dt_openpo = CONVERT(varchar(10), dateadd(d, -5,getdate()), 121)   
      AND stat_void = 0  
*/  
...
--<3.Data Prepare Area>
 ...
 --SELECT top 57 '#tmp_icim_comp',* FROM #tmp_icim_comp
   ...
   SELECT GROUP_CODE               = isnull(group_code,'')
         ...
         ,JUNIPER_PART_NUMBER      = CASE WHEN isnull(juniper_part_no,'') = 'None-Control' THEN '' ELSE isnull(juniper_part_no,'') END
       ...
       --,PO_OPEN_QUANTITY         = isnull(a.qty_buy,0.0)
         ...
         ,PO_PLACEMENT_DATE        = isnull(replace(CONVERT(char(10),g.dt_create,120),'-','/'),'') --modify by micky 20170928
     ...
     ELSE isnull(replace(CONVERT(char(10),a.dt_schedule,120),'-','/'),'')  END --modify by micky 20170928
	     ,AS_OF_DATE               = isnull(@DATE_GEN_STD,'') --CONVERT(varchar(10),GETDATE(),101) --modify by micky 20170928
 ...
 --SELECT COUNT(*) from #tmp_partner_out_data
   ...
   SELECT a.CM_PART_NUMBER,MANUFACTURER_NAME = c.grobal_vendor_name  
    ...
    WHERE a.CM_PART_NUMBER = b.part_no collate Chinese_PRC_Stroke_CI_AS              
      ...
      AND c.stat_void = 0   
      AND b.stat_void = 0 and b.stat_phaseout = 0  
    GROUP BY a.CM_PART_NUMBER,c.grobal_vendor_name  
   HAVING COUNT (*) = 1               
             
 --SELECT top 57 'table' = '#qvl_unique',* from #qvl_unique                  
  
   UPDATE a SET MANUFACTURE_PART_NUMBER = vendor_partno,MANUFACTURER_NAME = c.MANUFACTURER_NAME              
...
*/--[End]Insert into Real Table  
   ...
   SELECT ITEM_NAME = isnull(ltrim(rtrim(JUNIPER_PART_NUMBER)), '')
         ...
         ,SUPPLIER_SITE_CODE = gateway.dbo.CLRXMLInvalidRemove(PO_SUPPLIER_NAME)
 ...
 --SELECT * FROM #tmp_partner_out_data where CM_PART_NUMBER = '123400000504A'
